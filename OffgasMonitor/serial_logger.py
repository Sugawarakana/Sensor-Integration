...
def num(s):
    ...
    label (such as the 2 in CO2 or in H2) would be mistaken for the reading."""
    tail = s.split(":")[-1]
...
class Parser:
    ...
    (Voltage belongs to both ADC and SGX; Temperature/Humidity belong to both SGX and
    Telaire), so disambiguation has to rely on "which section are we currently in"
    rather than on the start of the line alone. A separator line clears the section,
    which prevents bleed-over between sections.
    """
 
    def __init__(self):
        self.row = None
        self.section = None      # adc / board / sgx / telaire / tamer / unknown
        self.in_flags = False
        self.flags_set = []
        self.flags_byte = 0
        self.reboots = 0
    ...
    def feed(self, line):
        ...
        # Board reboot: A: boot ... F: setup done — discard the half-finished row here
        ...
        # === Present in every round: ADC / I2C ===
...
class RowWriter:
    ...
    def write(self, row):
        ...
        self.count += 1
        if not self.quiet:
            print("  -> wrote row %d: %s %s | ADC %s | Tj %s | CO2 %s | CAN %s"
                  % (self.count, row["date"], row["time"], row["raw_adc"],
                     row["die_temp_c"], row["co2_ppm"],
                     row["can_device"] or "-"))
...
def selftest():
    ...
    expect = [
        # Round 1: everything normal, no SGX faults, both Telaire and Tamer present
        ...
        #          each bus; Telaire said nothing this second, so its columns must stay empty
        ...
        # Round 3: only half a round (the Ctrl+C case) — it should still be written out
        {"raw_adc": "100", "voltage_adc": "", "can_device": ""},
    ]
